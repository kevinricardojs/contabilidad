class PartidasController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @partida.update(partida_params)
        format.html { redirect_to libro_diario_partidas_path, notice: 'La Partida fue editada exitosamente.' }
      else
  ...
  def destroy
    @partida.destroy
    respond_to do |format|
      format.html { redirect_to libro_diario_partidas_path, notice: 'La Partida fue borrada.' }
    end
  end
